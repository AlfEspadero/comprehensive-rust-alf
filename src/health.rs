...
impl User {
	...
	pub fn visit_doctor(&mut self, measurements: Measurements) -> HealthReport<'_> {
		...
		let bp_change = match self.last_blood_pressure {
			None => {
				self.last_blood_pressure =
					Some((measurements.blood_pressure.0, measurements.blood_pressure.1));
				None
			}
			Some(bp) => Some((
				measurements.blood_pressure.0 as i32 - bp.0 as i32,
				measurements.blood_pressure.1 as i32 - bp.1 as i32,
			)),
		};
