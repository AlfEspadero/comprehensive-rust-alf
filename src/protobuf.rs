...
fn parse_field(data: &[u8]) -> (Field<'_>, &[u8]) {
	...
	let (fieldvalue, remainder) = match wire_type {
		...
		WireType::Len => {
			...
			let (value_bytes, remainder) = remainder.split_at(length);
			(FieldValue::Len(value_bytes), remainder)
		}
	};
	...
	fn add_field(&mut self, field: Field<'a>) {
		match field.field_num {
			...
			3 => {
				...
				let phone_number: PhoneNumber = parse_message(phone_data);
...
impl<'a> ProtoMessage<'a> for PhoneNumber<'a> {
	fn add_field(&mut self, field: Field<'a>) {
		match field.field_num {
			...
			2 => self.number = field.value.as_str(),
			_ => {}
		}
	}
}
